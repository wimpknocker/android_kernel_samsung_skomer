 ...
 * on critical errors.
 */
void db8500_prcmu_debug_dump(bool dump_abb)
{
	dump_stack();
...
struct prcmu_fops_register_data *__init db8500_prcmu_early_init(void)
{
	...
	pr_crit("%s: before armss_rate\n", __func__);
	flush_cache_all();
	ux500_clean_l2_cache_all();
	compute_armss_rate();
	pr_crit("%s: after armss_rate\n", __func__);
	flush_cache_all();
	ux500_clean_l2_cache_all();
...
static int __init db8500_prcmu_probe(struct platform_device *pdev)
{
	...
	myret = boot_disable_a9wdog(1);
	printk("%s: boot watchdog disabled: %d\n", __func__, myret);
